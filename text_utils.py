...
class RenderFont(object):
    ...
    #? tính toán từ kích cỡ font => nline , nch
...
#? mọi thứ ở trên được dùng trong render_sample
    def render_sample(self,font,mask):
        ...
        while i < self.max_shrink_trials and max_font_h > self.min_font_h:
            ...
            # render the text:
            #? render text and get bouding-box
            txt_arr,txt,bb = self.render_curved(font, text)
            #? chuyển từ (center,kc dài,kc rộng) - > 4 điểm x1y1,x2y2,x3y3,x4y4.
...
class FontState(object):
    ...
    def __init__(self, data_dir='data'):
        ...
        with open(font_model_path,'rb') as f:
            ...
            # file1 = open("font.txt", "w")
            # for i in self.font_model:
            #     file1.writelines(i + "\n")

            pkl_file = open('data.pkl', 'rb')
            self.font_model = cp.load(pkl_file)
        ...
        # self.FONT_LIST = osp.join(data_dir, 'fonts/ams.txt')

...
#?
        ...
        f = open(MASKS_DIR + 'gt_' +prefix + ".txt", "w+",encoding='utf8')
...
#? chỉnh sửa góc quay chữ
    ...
    # giới hạn góc quay tránh quay 180
    if R2d[0][0] < 0:
        R2d[0][0] = -R2d[0][0]
        R2d[1][1] = -R2d[1][1]
...
#? TODO :debug xem các giá trị để h
    ...
    def char2wordBB(self, charBB, text,itext):
        """
        ...
        return wordBB
    def word2SentenBB(self,wordBB,text):

        bb_idx = np.r_[0, np.cumsum([len(w.split()) for w in text])]
        senBB = np.zeros((2, 4, len(text)), 'float32')
        ...
        for i in range(len(text)):
            ...
            for pidx in range(perm4.shape[0]):
                d = np.sum(np.linalg.norm(box[perm4[pidx], :] - cc_tblr, axis=1))
                dists.append(d)
            senBB[:, :, i] = box[perm4[np.argmin(dists)], :].T
 ...
 #? quá trình render text vào hình, trả về (img , BB, Text, Mask(cùng kích thước với img)
    def render_text(self, imname,rgb, depth, seg, area, label, ninstance=1, ocrgen = False, fe = None):
        ...
        try:
            ...
            # find text-regions: Get region from segmentation which are good for placing
            regions = TextRegions.get_regions(xyz, seg, area, label)

            # find the placement mask and homographies:
            #? homographies: tao hiệu ứng để đặt ch
        ...
        for i in range(ninstance):
            ...
            #? sử dụng lại cái region
            NUM_REP = 2  # re-use each region three times:
            ...
            if placed:
                ...
                idict['wordBB'] = self.char2wordBB(idict['charBB'].copy(), ' '.join(itext),itext)
                idict['senBB'] = self.word2SentenBB(idict['wordBB'].copy(),itext)
                idict['masks'] = masks
